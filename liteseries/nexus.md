`_handlers.py` provides the public runtime API for launching thread-local SQLite connections, shutting them down cleanly, and decorating time-series fetchers with the local cache behavior.
...
`_util.py` contains lower-level helpers for time utilities, SQLite schema inspection, CREATE TABLE statement generation, and Arrow table normalization before ingest.
