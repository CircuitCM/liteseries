...
TABLE_EXISTS_IN_SCHEMA = "SELECT 1 FROM pragma_table_list WHERE schema = ?   AND name = ?   AND type = 'table' LIMIT 1"
...
def series_range_select(table_ref: str, req_cols: tuple, pidx: tuple, vidx, time_col, srange, erange):
    ...
    if sn and en:
        return f"SELECT {cl} FROM {table_ref} WHERE {ps}", vidx
    if sn:
        return f"SELECT {cl} FROM {table_ref} WHERE {ps} AND {time_col} <= ?", (*vidx, erange)
    if en:
        return f"SELECT {cl} FROM {table_ref} WHERE {ps} AND {time_col} >= ?", (*vidx, srange)
    # other is case 2
    return (f"SELECT {cl} FROM {table_ref} WHERE {ps} AND {time_col} >= ? AND {time_col} <= ?", (*vidx, srange, erange))
